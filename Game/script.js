...
function iniciarJuego() {
  ...
  fetch(
    ...
    .then(async (result) => {
      ...
      fetch(
        env.ENDPOINT_USER + "/jugadores/" + urlParams.get("j1"),
        requestOptions
      )
        .then((response) => response.text())
        .then((result) => {
          ...
          document.getElementById("u1").innerHTML =
            user1 + json.nombres + " " + json.apellidos;

          fetch(
            env.ENDPOINT_USER + "/jugadores/" + urlParams.get("j2"),
            requestOptions
          )
            .then((response) => response.text())
            .then((result) => {
              ...
              document.getElementById("u2").innerHTML =
                user2 + json.nombres + " " + json.apellidos;
            })
...
function tirarDados(tokenpass) {
  //dados
  jQuery.ajax({
    url: env.ENDPOINT_DADOS + "/tirar/2",
    headers: {
    ...
    if (u1 >= 100) {
      ...
      fetch(
        ...
        .then(async (result) => {
          ...
          var raw = JSON.stringify({ marcador: [u1, u2] });
          ...
          fetch(
            env.ENDPOINT_TORNEOS + "/partidas/" + urlParams.get("id"),
            requestOptions
          )
            .then((response) => response.text())
            .then((result) => {
